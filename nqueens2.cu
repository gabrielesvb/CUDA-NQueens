...
__global__ void nQueenKernel(int* solutions, int size, const int numThreads) {
	...
	//printf("%d", i);
	//printf("Xd = %d, Block %d, thread %d checking in...\n", blockIdx.x * numThreads + threadIdx.x, blockIdx.x, 
	
	solutions[blockIdx.x * numThreads + threadIdx.x] = solve(size, board, numThreads);
	//printf("\n%d size, block %d, thread %d, %d\n", size, blockIdx.x, threadIdx.x, solutions[blockIdx.x * numThreads + threadIdx.x]);
}
...
int main() {
	...
	printf("NumThreadsPerBlock = %d, NumBlocks = %d,d Size = %d\n", numThreads, numBlocks, numThreads*numBlocks);
...
__device__ int solve(int size, int board[N][N], const int numThreads) {
	...
	for (d = 0; d < depth; d++) {
		...
		k = x / div;
		//k = __float2int_rd(fdividef((float)x, powf((float)N, (float)(depth - d))));
		//printf("BlockID: %d, threadID:%d / k=%d, x = %d at d = %d, while div is %f\n", blockIdx.x, threadIdx.x, k, x, d, (float)x/div);
		if (is_attacked(board, k, d)) {
			//printf("\nBranch of thread %d conflicts (%d, %d)\n", threadIdx.x, k, d);
	...
	if (is_attacked(board, x % N, depth)) {
		//printf("\nBranch of threadd %d conflicts (%d, %d)\n", threadIdx.x, x % N, depth);
		return 0; // useless branch
	}
	...
	//printf("%d has frontier starting at [%d, %d]  and has had parent [%d, %d]\n", blockIdx.x, frontier[0].x, frontier[0].y, hehe, bebe);


SearchStart:
	/*
	if (blockIdx.x < 8)
		printf("Block %d, Thread %d - Frontier: [%d, %d]\n", blockIdx.x, threadIdx.x, frontier[0].x, frontier[0].y);
	*/
	while (cont > 0) {
		/*
		printf("\ndepth = %d\n", depth);
